...
class WaypointGenerator {
public:
    using Point = std::vector<float>;  // [x, y, z, yaw]
    ...
    struct FeasibilityLimits {
        ...
        double max_normal_acceleration = 2.2;    // m/s^2
        ...
        // Optional tangent-yaw rate check. Infinity disables it.
...
private:
    ...
    static float TangentYaw(const CurveState& state, float fallback_yaw) {
        const double dx = state.first_derivative.x;
        const double dy = state.first_derivative.y;
        if (dx * dx + dy * dy <= 1e-12) {
            // A vertical tangent has no defined XY heading. Holding the previous
            // yaw avoids atan2(0, 0) jumps.
            return fallback_yaw;
        }
        return static_cast<float>(std::atan2(dy, dx));
    }

    static Point MakePoint(const CurveState& state, float fallback_yaw) {
        return {
            static_cast<float>(state.position.x),
            static_cast<float>(state.position.y),
            static_cast<float>(state.position.z),
            TangentYaw(state, fallback_yaw)};
    }
...
public:
    ...
    std::vector<Point> GenerateCircleWaypoints(
        ...
        while (theta < 2.0 * PI() - EPSILON()) {
            ...
            waypoints.push_back(MakePoint(
                state,
                waypoints.empty() ? 0.0f : waypoints.back()[3]));
    ...
    std::vector<Point> GenerateLissajousWaypoints(
        ...
        waypoints.push_back(MakePoint(initial, 0.0f));
        ...
        while (u < period - EPSILON()) {
            ...
            waypoints.push_back(MakePoint(state, waypoints.back()[3]));
        }

        return waypoints;
    }

    /** 3D Lissajous with tangent yaw projected onto the XY plane. */
    std::vector<Point> GenerateLissajousWaypoints(
        float A,
        float B,
        float C,
